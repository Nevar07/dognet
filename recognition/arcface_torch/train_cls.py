...
def load_data(data_dir, input_size=224, batch_size=36):
...
def train_model(model,cls, train_loader, valid_loader, criterion, optimizer, args):
    ...
    def train(model, cls,train_loader, optimizer, criterion):
        model.eval()
        cls.train()
        total_loss = 0.0
        total_correct = 0
    ...
    def valid(model,cls, valid_loader, criterion):
        model.eval()
        cls.eval()
        total_loss = 0.0
        total_correct = 0
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_acc = train(model, cls,train_loader, optimizer, criterion)
        loss_dict['train'].append(train_loss)
        acc_dict['train'].append(train_acc)
        print("training: Loss:{:.4f}, Acc:{:.4f}".format(train_loss, train_acc))
        valid_loss, valid_acc = valid(model, cls,valid_loader, criterion)
        ...
        if valid_acc > best_acc:
            ...
            if torch.cuda.device_count() > 1:
                torch.save(best_model.module.state_dict(), os.path.join(save_dir, 'best_cls.pt'))
            else:
                torch.save(best_model.state_dict(), os.path.join(save_dir, 'best_cls.pt'))
            print(f'best classifier saved at epoch {epoch+1}')
            early_stop = 0
        else:
            early_stop +=1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str,default='./pretrain', help='dataset path')
    parser.add_argument('--save_dir', type=str,default='./models/vitb32_1/cls', help='model save path')
    parser.add_argument('--network', type=str, default='vitb32', help='model save path')
    ...
    ## data preparation
    train_loader, valid_loader = load_data(args.data_dir, input_size=input_size, batch_size=batch_size)
    ...
    weight_path = args.save_dir.strip('cls')+'best_model.pt'
    ...
    ## optimizer
    optimizer = optim.SGD(cls.parameters(), lr=lr, momentum=0.9)
